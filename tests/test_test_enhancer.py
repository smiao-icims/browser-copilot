...
5. Click submit"""
        ...
        assert any(keyword in suggestions_text.lower() for keyword in 
                  ['wait', 'verify', 'check', 'assert', 'screenshot'])
...
3. Click search button"""
    ...
    def test_suggest_improvements(self):
        ...
        assert any('specific' in s.lower() or 'selector' in s.lower() for s in suggestions3)
...
5. Fifth step"""
        ...
        assert len(result['errors']) > 0
        assert any('number' in e.lower() or 'sequence' in e.lower() 
...
4. Done"""
        ...
        assert len(result['warnings']) > 0
        assert any('verif' in w.lower() or 'check' in w.lower() 
...
3. Select the option"""
        ...
        assert len(result['warnings']) > 0
        assert any('specific' in w.lower() or 'selector' in w.lower() 
                  for w in result['warnings'])

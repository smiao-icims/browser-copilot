...
class AuthenticationStep(WizardStep):
        ...
        try:
            ...
            if registry.is_provider_configured("github_copilot"):
                ...
                try:
                    ...
                    provider_data = current_config.get("providers", {}).get("github_copilot", {})
            ...
            auth_command = ["uv", "run", "modelforge", "auth", "login", "--provider", "github_copilot"]
    ...
    def can_skip(self, state: WizardState) -> bool:
        ...
        return state.provider not in ["github_copilot"] + list(self.API_KEY_PROVIDERS.keys())
...
class ConfigManager:
    ...
    def validate(self) -> List[str]:
        ...
        if config.get("browser") not in valid_browsers:
            errors.append(f"Invalid browser: {config.get('browser')}. Must be one of: {', '.join(valid_browsers)}")
        
        # Validate output format
        valid_formats = ["json", "yaml", "xml", "junit", "html", "markdown"]
        if config.get("output_format") not in valid_formats:
            errors.append(f"Invalid output format: {config.get('output_format')}. Must be one of: {', '.join(valid_formats)}")
        ...
        for key in ["logs_retention_days", "reports_retention_days", "screenshots_retention_days"]:
        ...
        if config.get("compression_level") not in valid_compression:
            errors.append(f"Invalid compression level: {config.get('compression_level')}. Must be one of: {', '.join(valid_compression)}")

...
class SmartTrimStrategy(ContextStrategy):
    ...
    def create_hook(self) -> PreModelHook:
        ...
        def smart_trim_hook(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            max_single_message_tokens = self.config.window_size // 10  # No single message should take >10% of budget
            ...
            for i in range(len(message_infos) - 1, 0, -1):
                ...
                for idx in required_indices:
                    if idx not in included and message_infos[idx].tokens > max_single_message_tokens:
                        if self.verbose:
                            print(f"[Smart Trim Hook] WARNING: Message {idx} is very large ({message_infos[idx].tokens} tokens)")
                ...
                if token_count + group_tokens > self.config.window_size:
                    if self.verbose:
                        print(f"[Smart Trim Hook] Stopping at message {i} - would exceed budget ")
            ...
            if self.verbose:
                ...
                # Format results
                msg_reduction = ((len(messages) - len(result_messages)) / len(messages) * 100) if messages else 0
                token_reduction = ((original_tokens - token_count) / original_tokens * 100) if original_tokens else 0
                
                print(f"[Smart Trim Hook] === RESULTS ===")
                print(f"[Smart Trim Hook] Messages: {len(messages)} → {len(result_messages)} ({msg_reduction:.1f}% reduction)")
                print(f"[Smart Trim Hook] Tokens: {original_tokens:,} → {token_count:,} ({token_reduction:.1f}% reduction)")
                
                if excluded_messages:
                    print(f"[Smart Trim Hook] Excluded {len(excluded_messages)} messages")

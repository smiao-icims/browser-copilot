...
class SlidingWindowStrategy(ContextStrategy):
    ...
    def create_hook(self) -> PreModelHook:
        ...
        def sliding_window_hook(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if self.verbose:
                ...
                print(f"[Sliding Window] Preserve first: {self.config.preserve_first_n} Human/System messages")
            ...
            if total_tokens <= self.config.window_size:
                if self.verbose:
                    print(f"[Sliding Window] Under window size, keeping all messages")
                return {}
            
            # Step 1: Build tool dependency map  
            tool_dependencies = {}  # Maps message index to indices it depends on
            reverse_dependencies = {}  # Maps message index to indices that depend on it
                    ...
                    for tc in msg.tool_calls:
                        ...
                        if tc_id:
                            ...
                            for j in range(i + 1, len(messages)):
                                if isinstance(messages[j], ToolMessage) and getattr(messages[j], 'tool_call_id', None) == tc_id:
            ...
            if self.verbose and last_m_count > 0:
                last_m_tokens = sum(self.count_tokens(messages[i]) for i in selected_indices if i >= last_m_start)
                print(f"[Sliding Window] Preserved last {last_m_count} messages with integrity ({last_m_tokens:,} tokens)")
            
            # Step 4: Check if we have budget left for middle messages
            if current_tokens > self.config.window_size:
                if self.verbose:
                    print(f"[Sliding Window] WARNING: First {preserve_count} + Last {last_m_count} = {current_tokens:,} tokens (exceeds {self.config.window_size:,})")
                    print(f"[Sliding Window] No budget for middle messages")
            else:
                ...
                if self.verbose and remaining_budget > 0:
                    print(f"[Sliding Window] Remaining budget for middle messages: {remaining_budget:,} tokens")
            ...
            if self.verbose:
                self._print_results(messages, trimmed_messages, total_tokens, selected_indices)
    ...
    def _print_results(self, messages, trimmed_messages, total_tokens, selected_indices):
        ...
        print(f"[Sliding Window] Messages: {len(messages)} → {len(trimmed_messages)} ({msg_reduction:.1f}% reduction)")
        print(f"[Sliding Window] Tokens: {total_tokens:,} → {trimmed_tokens:,} ({token_reduction:.1f}% reduction)")
        
        # Show which messages were kept
        if len(selected_indices) < 20:  # Only show if reasonable number
            print(f"[Sliding Window] Kept indices: {sorted(selected_indices)} ({trimmed_tokens:,} tokens)")
        else:
            ...
            print(f"[Sliding Window] Kept ranges: {', '.join(ranges)} ({trimmed_tokens:,} tokens)")
